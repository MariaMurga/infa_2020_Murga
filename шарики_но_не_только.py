...
class triangle:
    ...
    def draw(self, surface): #рисует треугольник
        ...
        polygon(surface, self.color, [(self.x, self.y), (self.x + self.a, self.y) ,
                                      (self.x + self.a*(0.5), self.y + self.a*((0.75)**(0.5)))])
    ...
    def event(self, event, score): #проверяет, попал ли клик в треугольник
        ...
        if pos_y >= self.y and pos_y <= self.y + self.a*((0.75)**(0.5)):
            if pos_x >= (self.x + 0.5*self.a*(1 - ((self.y + self.a*((0.75)**(0.5))
                                                    - pos_y)/self.a*((0.75)**(0.5))))):
                if pos_x <= (self.x + self.a - 0.5*self.a*(1 - ((self.y + self.a*((0.75)**(0.5))
                                                                 - pos_y)/self.a*((0.75)**(0.5))))):
                    score += 15
            
        return score

    def size_change(self, dt): #меняет размер треугольника со временем

        self.a += 2*math.cos(dt) #за счёт косинуса размер то увеличиввется,то уменьшается 
...
while not finished:
    ...
    for i in range(5):
    	...
    	pool_tri[i].move()
    	pool_tri[i].size_change(2*time.time())
